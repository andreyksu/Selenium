...
public class BaseTestClass {
    ...
    public void createFolder(ITestContext testContext, Method method, String startPath) {
        ...
        _logger.debug("Full path to folder for save screen shot = '{}'", fullPath);
        try {
            path = Files.createDirectories(Paths.get(fullPath));
        } catch (IOException e) {
            _logger.error("Произошла ошибка при создании дирректории для хранения артефактов теста! Выполнение тестов невозможно!", e);
    ...
    /**
     * Добавляет ScreenShot из файловой системы - к отчету Allure.<br>
     * Ранее использовал данный метод. Сейчас заменил на - Allure.addAttachment(targetFile, inputStream);
     * 
